...
- **Shared JS** `Js/`: `reconcile-shared.js` (service ใหญ่สุด ~79 exports — cycle/Book/Match/adjustments), `script.js` (index เท่านั้น), `warehouses-shared.js` (registry คลัง), `audit-*.js` 4 ตัว (audit_check), `chat-notify-shared.js` (inject โดย sidebar ทุกหน้า)
