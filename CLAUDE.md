...
- ✅ **ปุ่ม "Export ยอดจริง" + กู้คืนการยืนยันรอบ 2026-08** (2026-08-11 บ่าย): admin ลบแถวซ้ำแล้วขอเริ่มยืนยันขาด-เกินใหม่ → ลบ acceptances 168 แถว + ยอดปรับ 2 แถวของรอบนี้ (log `RECONCILE_ADJ_CLEAR` ก่อนลบ · รอบเก่า 733 รายการไม่แตะ) แล้ว refresh · ฟีเจอร์ใหม่: ชีต `ยอดจริง` บอกต่อ SKU ว่าเชื่อเลขไหน (`buildFinalQtyExportRows`) — **แถวที่ยังไม่ตัดสินเว้นช่องว่าง ไม่เดาแทนคน** · เทส 397 PASS
  - ⚠️ **ความหมายของปุ่มยืนยันในหน้า Match** (ชื่อปุ่มเปลี่ยน 2026-08-13): "ยืนยันว่ายอด Excel ถูกต้อง (ไม่ปรับยอด)" = ยอด Excel ใช้เทียบคือยอดจริง (เขียน `reconciliation_match_acceptances`) · "ใช้ยอดที่กรอกเป็นยอดจริง (Apply)" = ยอดในช่องคือยอดจริง (สร้าง+Apply `stock_adjustments` ให้ Excel ใช้เทียบ = ยอดนั้น และถ้าต่างจากผลนับจะเขียน acceptance กำกับด้วย) — สองตารางนี้คือแหล่งความจริงของ "การตัดสิน" ทั้งหมด
...
- ✅ **หน้าใหม่ "ประวัติการปรับ / ยืนยัน"** (2026-08-13): แยก modal เดิมของ reconcile ออกเป็นหน้าเต็ม [`Html/adjust_history.html`](docs/pages/adjust_history.md) — ค้นหา SKU · กรอง 5 เกณฑ์ (ประเภท/SKU/รายละเอียด/ช่วงวัน/ช่วงจำนวน) · เรียง 4 คอลัมน์ · Export Excel+CSV · คืนค่า · หน้าเว็บ 11 → **12** · **เทส 459 PASS / 0 FAIL** (baseline 425) · **mutation 6 แบบแดงครบ 6**
  ...
  - smoke จริงบนข้อมูลรอบ 2026-08: 659 รายการ / 627 SKU · filter/sort/deep-link/selection-guard/Export ผ่าน · modal เดิมให้จำนวนเท่ากันเป๊ะ (659 แถว)
- ✅ **ปิด finding จาก code review 6 ข้อ** (2026-08-13): **เทส 474 PASS / 0 FAIL · mutation 14 แบบ แดงครบ 14**
  - 🔴 **High: `fetchAdjustments` / `fetchMatchAcceptanceMap` ไม่แบ่งหน้า** — PostgREST ตัดที่ 1,000 แถวเงียบ ๆ (**M1 รอบที่ 4**) กระทบทั้งหน้า Match และไฟล์ Export พร้อมกัน โดยจอยังบอก "แสดง N / N รายการ" ราวกับครบ · แก้แล้วพร้อม `.order('id')` ตาม invariant ข้อ 13 — ⚠️ `reconciliation_match_acceptances` **ไม่มีคอลัมน์ `id`** (PK = `cycle_id` + `sku_id`) จึงเรียง `sku_id` แทน และเพิ่ม `NO_ID_TABLES` ใน `stable-paging.test.mjs` (ยกเว้นราย**ตาราง** ไม่ใช่ราย**ไฟล์** — ALLOWLIST ระดับไฟล์จะปิดตาให้ query อื่นทั้งไฟล์)
  - ⚠️ **error path ต้องล้าง `viewEntries` ด้วย ไม่ใช่แค่ `allEntries`** — ไม่งั้นปุ่ม Export ค้างเปิด กดแล้วได้ข้อมูล**รอบก่อนหน้า**ในไฟล์ที่ชื่อบอกรอบผิด · เงื่อนไขเปิดปุ่มต้องดู `currentCycle` ด้วย ไม่ใช่แค่จำนวนแถว
  - ⚠️ **`fetchCycleById` คืน `null` ได้** (รอบถูกลบ/RLS) ต้องโยน error ไม่ใช่เดินต่อ — `buildHistoryFileName(null)` จะตกไปเป็น "คลังทั้งหมด" แล้วไฟล์ติดป้ายผิด
  - 🔴 **`tests/helpers/lift.mjs`: sandbox เป็น *สำเนา* ของ context** — ฟังก์ชันเขียนทับตัวแปรระดับโมดูล (`viewEntries = ...`) แล้วผู้เรียกอ่านไม่เห็น ⇒ เทสที่ตรวจ "สถานะหลังรัน" ผ่านแบบว่างเปล่า · เพิ่ม `liftInto()` คืน sandbox มาด้วย
  - 🔴 **`git add -A` กวาดการแก้ที่เราไม่ได้เขียนเข้า commit** — `Html/reconcile.html` ถูกกลไกอื่นแก้ (rename `.rc-field` → `.rc-filter-field`) ระหว่างเปิดค้างใน Browser pane แล้วหลุดเข้า `4905e42` · **ต้องดู `git diff --stat` เทียบกับที่ตั้งใจแก้ก่อน commit เสมอ** และเช็ค `git hash-object` ก่อน/หลังเปิดไฟล์ใน preview
  - ⚠️ เทสที่ slice ซอร์สด้วยเลขคงที่ (`slice(at, at+2200)`) จะมองไม่เห็นท้ายฟังก์ชันแบบเงียบ ๆ เมื่อโค้ดยาวขึ้น — ใช้ `bodyOf()` นับวงเล็บแทน
- ✅ **แผงปรับยอดหน้า Match: "ใช้ยอดที่กรอกเป็นยอดจริง"** (2026-08-13): ตามที่ admin สั่ง — (1) ถอดปุ่ม "บันทึก Draft" (ซ้ำกับ Apply · draft เหลือจาก "ปรับยอด Auto"+flow ชุดเท่านั้น) (2) เปลี่ยนชื่อปุ่ม "ยอมรับผลนับ (Apply ทันที)" → **"ใช้ยอดที่กรอกเป็นยอดจริง (Apply ทันที)"** เพราะชื่อเดิมอ่านได้ว่าใช้ผลนับดิบทั้งที่จริงใช้ค่าในช่อง (3) ปุ่มเขียว → **"ยืนยันว่ายอด Excel ถูกต้อง (ไม่ปรับยอด)"** เพราะ admin เข้าใจกลับด้านว่าเชื่อผลนับ (4) **ปิดช่องว่าง: กรอกยอด ≠ ผลนับ (เช่นทีมงานตรวจซ้ำได้ 450 ทั้งที่นับไว้ 200) แล้ว Apply — เดิมสถานะค้างขาด/เกิน + toast โกหกว่า "สถานะถูกต้อง"** ตอนนี้เขียน `reconciliation_match_acceptances` กำกับอัตโนมัติ + note เก็บทั้งยอดที่กรอกและผลนับ · ตรวจข้อมูลรอบ 2026-08 ใน DB จริง: ยอดปรับ applied 659 รายการ **จบที่ตรงผลนับครบทุกแถว ไม่มีแถวเพี้ยน** (เคสกรอกต่างผลนับยังไม่เคยเกิด) · **เทส 483 PASS / 0 FAIL · mutation 3/3 แดง** · เทสใหม่รัน `acceptCountedForLine` + `acceptCountedApplyFromPanel` + `renderAdjPanelSummary` + `resetAcceptApplyBtn` จริง (`tests/dryrun/adjust-panel-truth.test.mjs`)
  - ⛔ **ป้าย "ต้องปรับ Book" ถูกถอดออกถาวร** (มีเทสห้ามคำนี้กลับมาทั้งไฟล์) — admin เข้าใจผิด 2 ครั้งว่าระบบไปแก้ยอดในไฟล์ Excel · ป้ายใหม่ = **"ส่วนต่างที่ต้องบันทึกปรับ / ไม่แก้ไฟล์ Excel"** · ความจริง: ปุ่มปรับยอดทุกตัว **INSERT แถวใน `stock_adjustments` อย่างเดียว** — `book_qty` ไม่เคยถูกเขียนทับ (ยืนยันกับฐานจริง: `BG001` book_qty ยัง = 100 เท่าไฟล์เดิม ขณะ `adjusted_book_qty` = 325) · ตารางเต็มอยู่ใน [docs/pages/reconcile.md](docs/pages/reconcile.md)
  - 🔴 **บทเรียน `tests/helpers/lift.mjs`**: ฟังก์ชันที่มี default parameter เป็น object (`opts = {}`) ถูก `bodyOf` ตัดขาดกลางอากาศ — `{` แรกหลังชื่อคือของ param ไม่ใช่ body ⇒ เพิ่ม `fnSlice` ข้าม parameter list ก่อน (เจอเพราะเทสใหม่พัง ไม่ใช่เพราะเทสเก่าเตือน)
  - ⚠️ **บทเรียน mutation test**: ห้ามใช้ `git checkout --` กู้ไฟล์ระหว่าง mutation ก่อน commit — มันดึงของ HEAD มาทับการแก้ทั้งหมด ให้ `cp` จาก backup เท่านั้น (เกือบพลาดจริง 2026-08-13 กู้ทันจาก backup)
- ✅ **ตัวช่วยกดปุ่มเดียว + popup "?" จำลองผลลัพธ์** (2026-08-13): **เทส 496 PASS / 0 FAIL · mutation 3/3 แดง**
  - ปุ่ม 2 ตัวตั้งชื่อใหม่ให้เป็น**คู่ขนาน + มีตัวเลขจริงบนปุ่ม**: `ใช้ยอด Excel (278) เป็นยอดจริง` / `ใช้ยอดที่กรอก (197) เป็นยอดจริง` (ชื่อเปลี่ยนมา 3 รอบแล้ว — ประวัติและเหตุผลอยู่ใน [docs/pages/reconcile.md](docs/pages/reconcile.md) **ห้ามเปลี่ยนกลับ** มีเทสห้ามชื่อเก่า)
  - `resolveAdjButtonGuidance` ไฮไลต์กรอบแดงปุ่มที่ควรกด + หรี่ปุ่มที่ไม่เกี่ยว · **หรี่แล้วยังกดได้โดยตั้งใจ** (ระบบเตือนได้ ห้ามบล็อกคน — นโยบายข้อ 3) · ปุ่ม `?` เปิด `buildActionPreview` บอกล่วงหน้าว่ากดแล้วยอดจริงเท่าไร ยอดปรับเท่าไร ต้องแก้ระบบต้นทางเท่าไร
  - ⛔ **เกณฑ์ไฮไลต์ต้องเทียบ `target` กับ Excel ใช้เทียบ ไม่ใช่ผลนับ** — ข้อเสนอแรกของ admin ("ช่อง = ผลนับ ⇒ หรี่ปุ่มกรอก") กลับด้าน เพราะค่าเริ่มต้นของช่าง**คือผลนับ** จะหรี่ปุ่มที่ถูกต้องในเคสปกติที่สุดแล้วชี้ไปปุ่มที่ทิ้งผลนับ · เทส `[guidance] ⛔ เคสค่าเริ่มต้น` บังคับไว้
  - 🔴 **`Number('') === 0` ไม่ใช่ NaN** — ช่องว่างเคยถูกอ่านเป็น "ยอด 0" แล้วไฮไลต์ปุ่มผิด · เทสจับได้ตอนเขียน ไม่ใช่ตอน review (เจอครั้งแรกในโปรเจกต์)
  - ⚠️ **ซ่อน/โชว์ปุ่มต้องทำที่ `.rc-act-wrap` ไม่ใช่ที่ปุ่ม** — ไม่งั้นปุ่ม `?` ลอยเดี่ยว
  - 🔴 **mutation ที่ "รอด" ต้องเช็คก่อนว่าใส่ติดจริงไหม** — สตริงหลายบรรทัดไม่แมตช์แล้วเทสเขียวหมด ดูเหมือนเทสไม่ยาม ทั้งที่ยามอยู่ (พอใส่ถูกแดง 5 ข้อ) · **ให้ `assert old in s` ในสคริปต์ mutation เสมอ**
- ✅ **"เหลือปุ่มเดียว" ในแผงปรับยอด + เทสเคสนับได้ 0** (2026-08-13): **เทส 511 PASS / 0 FAIL · mutation แดงตามคาด**
  - ขาด/เกิน → โผล่**เฉพาะปุ่มกรอก** · `book_only` → โผล่**เฉพาะปุ่ม Excel** · เพิ่มทางลัด「ใส่ตามยอด Excel」คู่กับ「ใส่ตามผลนับ」 · เลิกใช้การ "หรี่ปุ่ม" (ซ่อนไปเลยแรงกว่า) แต่คงกรอบแดงชี้ปุ่มที่ควรกด
  - **พิสูจน์แล้วว่าปุ่ม Excel ซ้ำซ้อน 100% ตอนขาด/เกิน** — พิมพ์เลข Excel ลงช่องแล้วกดปุ่มกรอก จะวิ่งเข้าเส้นทาง `noAdjustment` เขียน acceptance แถวเดียวเหมือนกันเป๊ะ · ในฐานมีแถวจากเส้นทางนี้จริง 4 แถว
  - สถิติการใช้จริงก่อนแก้: ปุ่ม Excel ถูกกด 76 ครั้ง — **`book_only` 54 ครั้ง (71%)** · ขาด/เกิน 21 ครั้ง (ซ้ำซ้อน) — ดังนั้นการซ่อนตอนขาด/เกินไม่ทำให้เสียความสามารถอะไร
  - 🔴 **เจอข้อมูลเสียจริงจากปุ่มเดิม (ยังไม่แก้)** — note ของ acceptance ใช้ "เลขในช่อง" ไม่ใช่ยอดที่บันทึกจริง · รอบคลังอะไหล่ 2026-06 มี **5 แถวที่ note บอกเลขผิด** เช่น `SP069` note `ยืนยันถูกต้อง @ 30` แต่ยอดจริงที่ระบบบันทึกคือ **68** (ต่างกัน 38 ชิ้น) · แถวที่มาจากปุ่มกรอก (`ยอมรับ effective N — ผลนับ M`) ถูกต้องหมด · **ต้องแก้ทั้งโค้ดและข้อมูลเก่า**
  - เทสใหม่ `tests/dryrun/zero-count-flows.test.mjs` ตอบโจทย์ "นับได้ 0" — แยก 2 แบบชัด: **มีแถวนับ+จำนวน 0 → `short` ยอมรับขาดเป็นชุดได้** (25 SKU ในฐาน) · **ไม่มีแถวนับเลย → `book_only` ระบบไม่เลือกให้** (70 SKU) · ตัวแยกคือ `hasCountRecord` ไม่ใช่ตัวเลข 0
  - ปรับยอด Auto ตรวจครบ 6 ข้อ: สร้าง draft ถูกทิศ · ไม่แตะ `book_only` · ข้ามที่ยืนยันแล้ว/มี draft · โหมดเกินไม่กวาดขาด · ยกเลิกแล้วไม่เขียน · โหมด "ดูทั้งหมด" ไม่ทำงาน
- ✅ **เลิกใช้คำว่า "Excel ใช้เทียบ" + หมายเหตุ acceptance จดยอดจริง** (2026-08-13): **เทส 523 PASS / 0 FAIL · mutation 2/2 แดง**
  - ⛔ **`effective_book_qty` ไม่ใช่เลขในไฟล์ Excel** — มันคือ `book_qty + ผลรวมยอดปรับ` · ชื่อเดิมทำให้ admin เข้าใจผิดมาตลอดว่าเท่ากับยอดในไฟล์ ทั้งที่ **56–74% ของ SKU ในรอบจริงไม่เท่ากัน** ⇒ เปลี่ยนเป็น **"ยอดตั้งต้นตอนนี้"** ทั้งหน้า (35 จุด) · มีเทสห้ามคำเก่ากลับมา
  - ⛔ **`buildMarkMatchNote` ต้องจากยอดที่บันทึกจริง (`effective`) ห้ามจากช่องกรอก** — ของเดิมจด `ยืนยันถูกต้อง @ ${target}` โดย target ตั้งค่าเริ่มต้นเป็น**ผลนับ** จึงจดเลขที่ถูก*ปฏิเสธ*ไว้แทน · **ข้อมูลเสียจริง 23/76 แถว** (`SP069` จด `@30` ทั้งที่ระบบบันทึก 68 · รอบ 2026-05 มี 11 แถวที่จดเลขซึ่งไม่ใช่ทั้งยอดตั้งต้นและผลนับ = คนพิมพ์เลขไว้แล้วถูกทิ้ง) · **ข้อมูลเก่ายังไม่ได้แก้ — admin เลือกทำแค่ข้อ 1-2**
  - 🔴 **`Number('') === 0` พลาดครั้งที่ 2 ในวันเดียว** — รวมเป็น `parseQtyOrNull()` ตัวเดียวแล้ว ห้ามเขียน guard เองซ้ำ
  - 🔴 **มีงาน design token (Light/Dark) ทำคู่ขนานอยู่คนละสาย** — `git diff` โผล่ `chat.html` / `count_search.html` / `import_counts.html` ที่ไม่ใช่ของเรา · **ต้อง `git add` ราย path เสมอ ห้าม `-A`** และเช็คว่าไฟล์ตัวเองไม่ถูกปน (grep หา `data-theme-ready` / `var(--surface` ใน diff ของตัวเอง)
- ✅ **ระบบธีม Light/Dark ทั้งเว็บ (Phase 1–6)** (2026-08-13): ตกแต่งเท่านั้น ไม่แตะ logic/DB — **เทส 523 PASS / 0 FAIL**
  - **สถาปัตยกรรม**: `Css/style.css` = design token กลาง (`:root` = dark slate ใหม่ `#0f141b` เป็นค่าเริ่มต้น + `html[data-theme="light"]` override ครบทุก token) · ปุ่มสลับท้ายเมนูซ้าย (`Js/sidebar-shared.js` — เขียน `localStorage.theme_v1` + ประกาศ event `themechange`) · boot script inline ใน `<head>` ทุกหน้า **ก่อน** `<link>` style.css กัน flash
  - ⛔ **กติกาใหม่: ห้ามเขียนสี hex/rgba ดิบในหน้า HTML — ใช้ `var(--token)` เสมอ** และเพิ่ม token ต้องกำหนดครบทั้ง 2 ธีม · เทสยาม `tests/unit/theme-guard.test.mjs` 6 ข้อบังคับ (boot ครบ/token คู่ light ครบ/หน้า ready hex ≤3/Outfit ห้ามหาย/ปุ่มห้าม onclick string/จอ TV dark ถาวร)
  - **กลไก phased rollout**: หน้าเสร็จติด `data-theme-ready` บน `<html>` — ไม่มี attr = boot บังคับ dark เสมอ (ผู้ใช้เลือก light ค้างไว้ก็ไม่พัง) · ready **ครบทุกหน้า** ยกเว้น `live_count_wall` ที่ตั้งใจ dark ถาวร (จอ TV มีเทสบังคับ)
  - **Chart.js อ่านสีผ่าน `DS.chartColors()`** (getComputedStyle ตอนวาด — ห้าม freeze ตอน module load) + dashboard ฟัง `themechange` แล้ววาดใหม่ทุกกราฟ · `MATCH_STATUS_LABELS.color` เป็นค่า dark คงที่ (compat) กราฟห้ามใช้ตรง ๆ
  - **โซนสเปรดชีตของ audit_check ใช้ token `--sheet-*`** — กระดาษขาวแบบ Google Sheets ค่าเท่ากันทั้ง 2 ธีมโดยตั้งใจ อย่า map เข้า surface กลาง
  - `reconcile.html` migrate เป็นลำดับสุดท้าย (131 รายการ) — ตอนแรกเลื่อนเพราะมี session อื่นแก้ไฟล์นี้ค้างอยู่ รอ commit `cce205b` ของงานนั้น land ก่อนแล้วค่อยทำ (working tree ต้องสะอาดก่อนแตะไฟล์ที่งานอื่นถืออยู่) · ⚠️ ไฟล์นี้ working copy เป็น CRLF — สคริปต์แทนสตริงข้ามบรรทัดด้วย `\n` จะไม่แมตช์เงียบ ๆ (เจอแล้ว 2 ครั้ง: style.css, reconcile.html)
  - ⚠️ วัด computed color ระหว่าง CSS transition จะได้ค่ากลางทาง — ปิด transition ก่อนวัดเสมอ (เจอตอน smoke import_counts)
  - cache-buster ปัจจุบัน: `ASSET_VER = '20260813d'` (bump 3 รอบใน 1 วันเพราะแตะ style.css หลายเฟส — แต่ละรอบต้องอัปเดต `?v=` ครบ 12 หน้า)
- ✅ **Supabase keep-alive** (2026-08-13): `.github/workflows/supabase-keepalive.yml` ยิง SELECT อ่านอย่างเดียว 1 แถว (`count_cycles`) ทุกจันทร์+พฤหัส 10:00 น. ไทย กัน free tier พักโปรเจกต์ · **ห้ามเปลี่ยนเป็นวิธี insert/ลบข้อมูลหลอก** (ขัด invariant ข้อ 1 — read ก็นับเป็น activity อยู่แล้ว) · URL/KEY อ่านสดจาก `Js/api.js` ด้วย `grep -a` (ไฟล์มี byte `\0` หลง grep ปกติมองเป็น binary) ไม่ต้องตั้ง secret · ping fail = GitHub ส่งอีเมลแจ้งอัตโนมัติ · มี step re-enable ตัวเองกัน GitHub ปิด scheduled workflow หลังรีโปเงียบ 60 วัน · ทดสอบรันจริงผ่านแล้ว (run 31689077658 = success)
