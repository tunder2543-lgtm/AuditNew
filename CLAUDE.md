...
- ✅ **ระบบธีม Light/Dark ทั้งเว็บ (Phase 1–6)** (2026-08-13): ตกแต่งเท่านั้น ไม่แตะ logic/DB — **เทส 523 PASS / 0 FAIL**
  ...
  - **กลไก phased rollout**: หน้าเสร็จติด `data-theme-ready` บน `<html>` — ไม่มี attr = boot บังคับ dark เสมอ (ผู้ใช้เลือก light ค้างไว้ก็ไม่พัง) · ตอนนี้ ready **11/12 หน้า** — `live_count_wall` ตั้งใจ dark ถาวร (จอ TV มีเทสบังคับ)
  ...
  - ⚠️ **`Html/reconcile.html` ยังไม่ migrate** (หน้าเดียวที่เหลือ) — ตอนทำธีมมีงานอื่นแก้ไฟล์นี้ค้างอยู่ (ปุ่ม/note adjust panel, diff +108 บรรทัดนอก commit ธีม) จึงเลื่อนเพื่อไม่ชนกัน · ทำต่อ: กวาด literal ~97 hex + 115 rgba เป็น token ตามแบบหน้าอื่น + ติด `data-theme-ready` · ระหว่างนี้หน้านี้ dark เสมอ + ปุ่มสลับ disabled อัตโนมัติ
