...
### - [x] M8. index: กติกากันซ้ำตอนแก้ไขขัดนโยบาย DB + เช็คจาก cache บางส่วน
> **✅ แก้แล้ว 2026-08-11** — กติกาเดิมผิด 2 ชั้นพร้อมกัน: **บล็อกงานที่ถูกต้อง** และ **ปล่อยงานที่ผิดจริงผ่าน**
> 1. บล็อกทุกแถวที่ SKU+ตำแหน่ง+คลังตรงกัน **โดยไม่ดูจำนวน** — ขัด invariant ข้อ 3 (นับแยกถุงคือการทำงานปกติ) และขัด migration 011 ที่อนุญาตระดับ DB
> 2. เช็คจาก `allRecords` ที่โหลดเฉพาะ scope ⇒ เลือกคลังคนละอันได้คำตอบคนละอย่าง และมองไม่เห็นแถวที่ชนกันจริง
> ตอนนี้ถาม DB จริง (`.eq('sku_id')` + `.eq('counted_qty')` · กรอง location ฝั่ง client เพราะฐานจริงมีตัวพิมพ์เล็กปน) แล้วส่งต่อให้ `AuditDedupe.classifyDestinationCollision` — **เกณฑ์เดียวกับหน้า audit_check** (index โหลด `audit-dedupe.js` เพิ่ม) จะได้ไม่ตัดสินคนละแบบ
> พ่วง: ตรวจตอน**แก้จำนวน**ด้วย ไม่ใช่เฉพาะตอนย้ายตำแหน่ง (แก้จำนวนให้ไปตรงกับแถวที่มีอยู่ ก็สร้างแถวซ้ำได้เหมือนกัน) · แถวที่ยังไม่ผูกรอบจำกัดด้วยเดือนไทยเหมือน M12 · ตรวจไม่สำเร็จ = บล็อก
> ยืนยันกับข้อมูลจริง: `BNP20 @ B2-01` มี 70 กับ 200 — เดิมแก้แถวไหนก็โดนบล็อก ตอนนี้แก้เป็นค่าอื่นผ่าน แก้ให้ไปตรงกับอีกแถวถึงจะบล็อก
> **review จับเพิ่ม 4 จุด (แก้แล้วในชุดเดียวกัน):**
> 1. 🔴 **query ไม่มี paging** — PostgREST ตัดที่ 1,000 แถวเงียบ ๆ และเพราะเรียงตาม `id` ขึ้น แถวที่ถูกตัดคือ**แถวใหม่สุด** ซึ่งคือแถวรอบปัจจุบันที่ guard มีไว้จับพอดี (บทเรียน M1 ซ้ำ) · เพิ่ม `.range()` + เพดาน 50 หน้า
> 2. 🔴 **race หลังกลายเป็น async** — `handleEdConfirm` มี 4 round-trip นำหน้าการเขียน แต่ไม่มียามกดซ้ำ/ไม่ disable ปุ่ม ⇒ ดับเบิลคลิกได้ audit log ซ้ำ · กดยกเลิกแล้วเปิดแถวอื่นระหว่างรอ ได้ **audit log ของการแก้ไขที่ไม่เคยเกิด** (ขัด invariant ข้อ 1) · เพิ่ม `edBusy` + `edGeneration` + ยาม stale หลัง await ทุกจุด
> 3. **คลังว่าง** เดิมยอมรับทุกคลังเป็นปลายทาง ⇒ ตัดสินไม่ได้แต่ปล่อยผ่าน · ตอนนี้บล็อกแบบเดียวกับ audit_check
> 4. เทียบชื่อคลังต้อง normalize ให้ตรงกับ audit_check · `id` ว่าง short-circuit ก่อนยิง query
> ⚠️ **mutation เปิดเผยความเปราะเพิ่ม**: ถ้า `.range()` หลุดหายไป ลูปจะวนไม่จบจนเบราว์เซอร์ค้าง — ใส่เพดานรอบไว้แล้ว
...
### - [x] M10. index: KPI แสดง 0%/0 ระหว่าง Book โหลด โดยแยกไม่ออกจาก "นับครบแล้ว"
> **✅ แก้แล้ว 2026-08-11** — แยก 3 สถานะ: กำลังโหลด → `—` (พร้อม tooltip) · ไม่มีรายการ Book → `—` + "เทียบไม่ได้" · พร้อมแล้ว → ตัวเลขจริง
> แก้ค่าเริ่มต้นใน markup ด้วย — ก่อน JS รันผู้ใช้ก็เห็น `0` / `0%` อยู่แล้ว จึงโกหกตั้งแต่วินาทีแรก
> **review จับเพิ่ม 2 จุด:** `refreshUncounted` (เส้นทางที่ทำงานเมื่อกดกล่อง KPI) เขียนทับ badge เป็นเลขดิบโดยไม่รู้จัก `bookPending` ⇒ ผู้ใช้ที่เห็น `—` แล้วกดดูรายการ จะได้ `0` กลับมา แล้วรายการขึ้นว่า "ยังไม่มี Book — อัปโหลดที่เมนูตั้งค่ารอบ" ทั้งที่ไม่ต้องทำอะไร · และเคส "รอบไม่มี Book" เดิมกล่องซ้ายขึ้น `0` กล่องขวาขึ้น `—` = สองกล่องบอกคนละเรื่อง · ตอนนี้ทั้งสองเส้นทางพูดตรงกัน
> ⚠️ **เทส M10 รุ่นแรกเป็น regex ล้วน ยามไม่ได้เลย** — review พิสูจน์ว่าใส่ `const bookPending = false;` แล้วเทสยังเขียว 348/348 · เปลี่ยนเป็นยก `updateStats`/`refreshUncounted` มารันจริงกับ DOM ปลอม
...
### - [x] M32. ไม่มี circuit breaker ตอน insert รายแถว
> **✅ แก้แล้ว 2026-08-11** — ทั้ง `insertGroupRowsOneByOne` (index) และ `importRowsOneByOne` (import_counts) หยุดเมื่อเจอ network error ติดกัน 3 ครั้ง แล้วโยนแถวที่เหลือเข้า `failed` พร้อมเหตุผลตรง ๆ ไม่ใช่หายเงียบ
> error ที่**ไม่ใช่**เน็ต (ข้อมูลผิดรายแถว) ยังลองให้ครบเหมือนเดิม — และความสำเร็จคั่นกลางรีเซ็ตตัวนับ
> ⚠️ แถวที่ถูกข้ามฝั่ง import ต้องพา `client_request_id` / `import_batch_id` เดิมไปด้วย ไม่งั้นกด "นำเข้าแถวที่เหลือ" แล้วแทรกซ้ำ (บั๊ก M6 กลับมา) · มีเทสบังคับ
> **review จับได้ว่า `aborted` เป็นค่า write-only** — chunk loop ไม่ได้อ่านเลย ⇒ ไฟล์หมื่นแถว = 50 chunk × 4 request ทั้งที่เน็ตตายตั้งแต่ chunk แรก (ลดจาก 10,050 เหลือ ~200 request ไม่ใช่หยุดจริง) · ตอนนี้ `break` แล้วโยนแถวของ chunk ที่ยังไม่ได้ยิงเข้า `failedRows` พร้อมคีย์เดิม
> พ่วง: toast ของหน้านับบอกด้วยว่า "ระบบหยุดบันทึกเอง" ไม่ใช่ปล่อยให้ผู้ใช้เดาจาก error ของแถวแรก
