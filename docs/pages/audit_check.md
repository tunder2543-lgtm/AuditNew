...
### Scope bar (`:979-999`)
คลัง · เดือน (เฉพาะเดือนที่มีข้อมูล) — เปลี่ยนค่าแล้ว auto รัน pipeline `runFullAuditLoad` (`:4023`): โหลด reference → เติมตาราง → verify ทุกแถว
...
3. **เทียบ Location กับ Excel** (modal เต็มจอ): โหลด template จาก Supabase Storage → อัปโหลด A=SKU, B=Location → **เลือกคลังในโมดัล** + เลือกโซน → [`Js/audit-loc-compare.js`](../../Js/audit-loc-compare.js) `buildLocComparePlan` แยกเป็น ไม่ตรง/ตรงอยู่แล้ว/ไม่มีในระบบ/กำกวมใน DB/**ข้อมูลไม่ครบ** → review → apply 2 ขั้น → export แถวที่ข้าม
   ...
   > - ต้องผ่านด่านตรวจให้ครบ (เดือน / SKU ซ้ำในไฟล์) **ก่อน** จะไปแตะ scope ของหน้า ไม่งั้นผู้ใช้กดยกเลิกแล้วหน้ายังเปลี่ยนคลังค้างไว้ · ถ้าเดือนถูกสลับอัตโนมัติต้องเตือน
...
## ตรวจ "ซ้ำข้ามตำแหน่ง" (เพิ่ม 2026-08-11)

**สถานะใหม่ `crossloc` (สีม่วง)** — SKU เดียวกัน **จำนวนเท่ากันเป๊ะ** แต่คนละตำแหน่ง ในรอบนับ + คลังเดียวกัน

ทำไมต้องเตือน: `refresh_reconciliation_for_cycle` รวมยอดด้วย `SUM(counted_qty)` ต่อ SKU ต่อรอบ **โดยไม่สนตำแหน่ง** ⇒ ถ้าเป็นการนับชั้นเดิมซ้ำใต้ป้ายคนละชื่อ ยอดจะถูกบวกซ้ำเต็ม ๆ

พบจากข้อมูลจริง (admin เจอด้วยตาก่อน) — เป็นชุดชัดเจน:

| SKU | ตำแหน่ง | นับได้ | Book | Match |
|---|---|---|---|---|
| PK089 | K3-03 · L4-03 | 256 + 256 | 258 | เกิน 254 |
| PK088 | K3-03 · L4-03 | 68 + 68 | 36 | เกิน 100 |
| PC697 | G1-02 · G2-05 | 329 + 329 | 331 | เกิน 327 |

K3-01/L4-01, K3-02/L4-02, K3-03/L4-03 … คือชั้นเดียวกันถูกนับ 2 ป้าย

- ตัวตรวจ: `AuditDedupe.findCrossLocationDuplicates` (`Js/audit-dedupe.js`) — เทส `tests/unit/cross-location-dup.test.mjs`
- ดัชนี `crossLocByRecordId` สร้างใน `buildCrossLocIndex()` ทุกครั้งที่โหลดข้อมูลอ้างอิง
- ลำดับความสำคัญใน `verifyRow`: ซ้ำในรอบเดียวกัน (แดง) → **ซ้ำข้ามตำแหน่ง (ม่วง)** → ทับซ้อน (ส้ม)
- ⚠️ **เตือนอย่างเดียว ห้ามลบ ห้ามแนะนำให้ลบ** (นโยบายข้อ 3) — SKU เดียวกันวางจริง 2 ที่แล้วจำนวนบังเอิญเท่ากันก็เป็นไปได้ · มีเทสบังคับว่าห้ามมีคำว่า "ลบ" ในข้อความ
- ปิดเสียงได้ด้วยปุ่ม **"ยืนยันว่าปกติ"** เดิม — คีย์คือ `ตำแหน่งทั้งหมด@จำนวน` ⇒ 1 กลุ่ม = 1 แถวใน `inventory_count_acceptances` · ข้อมูลเปลี่ยนแล้วกลับมาเตือนเอง
- **จับคู่ให้อยู่ติดกัน**: กดการ์ด "ซ้ำข้ามตำแหน่ง" แล้วระบบสลับโหมดเรียงเป็น `crossloc` อัตโนมัติ (คู่ของ PK011 เคยอยู่แถว 941 กับ 1005 คนละหน้าจอ) · มีเส้นคั่นสีม่วงระหว่างคู่ · ปิดตัวกรองแล้วคืนค่าเรียงเดิม · เลือกโซนไม่เด้งออกจากโหมดนี้
- ไม่นับ `counted_qty = 0` (ศูนย์ที่สองที่ไม่ใช่การนับซ้ำ และไม่กระทบ Match)

## โหมด "แก้ไขจำนวน" (เพิ่ม 2026-08-11)

ปุ่ม **แก้ไขจำนวน** ข้างปุ่ม "แก้ไขตำแหน่ง" — โครงเดียวกันทุกจุด:

- แก้เฉพาะแถวที่โหลดจากระบบ (มี record id) · SKU/ตำแหน่งล็อก · จำนวนต้องเป็นเลขจำนวนเต็ม ≥ 0
- ยืนยัน 2 ขั้น → guard `validateDestUpdateBatch` (M12 ตัวเดียวกับทุกทางแก้ไข — จำนวนใหม่ไปตรงกับแถวอื่นในรอบ+คลัง+SKU+ตำแหน่งเดียวกัน = สร้างแถวซ้ำจริง ถูกบล็อก)
- เขียน `inventory_audit_logs` action `AUDIT_EDIT_QTY` (ค่าเดิม → ค่าใหม่) ทุกแถว — ย้อนดูได้ในปุ่ม "ประวัติการแก้ไข/ลบ" และ drawer ประวัติหน้า index
- **ออกจากโหมดแล้ว re-verify ทั้งตาราง** กับข้อมูลอ้างอิงที่โหลดใหม่ ⇒ แถวที่แก้สำเร็จขึ้น "ข้อมูลถูกต้อง" ไม่ค้างที่ "ตำแหน่ง/จำนวนไม่ตรง" อีก (นี่คือโจทย์ของ admin)
- โหมดแก้จำนวน/แก้ตำแหน่ง/สลับ SKU เปิดได้ทีละอย่าง (readOnly ของช่องตีกัน)
- นโยบายข้อ 3 ห้าม**ระบบ**แก้เอง — การแก้โดยคนผ่านยืนยัน 2 ขั้น + audit log เป็นเส้นทางเดียวกับโหมดแก้ตำแหน่งที่มีอยู่แล้ว
- เทส: `tests/unit/edit-qty-mode.test.mjs` (11 ข้อ รันจริงผ่าน lift รวม `createRow`)
