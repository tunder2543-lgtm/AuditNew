...
- **[ยืนยันแล้ว] โหมดทุกคลังคูณ Book ต่อจำนวนคลัง** (`:2396-2404`): loop คลังแล้วบวก `bookList.length` (Book ทั้งรอบ ไม่กรองคลัง) ต่อคลัง → `totalSku`, "ยังไม่ได้นับ", progress bar ผิดเป็นเท่าตัวของจำนวนคลัง — เพิ่มคลังแปลก ๆ จากข้อมูล 1 แถวก็ทำให้ KPI กระโดดทั้ง Book
...
- ป้าย "รวม 3 คลัง" hardcode (`:2443`) ทั้งที่ registry เป็น dynamic
