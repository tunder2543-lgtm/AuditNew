...
### ⛔ ห้ามเรียกยอดตั้งต้นว่า "Excel ใช้เทียบ" อีก (เปลี่ยนชื่อ 2026-08-13)

`effective_book_qty` = **`book_qty` + ผลรวมยอดปรับ** ไม่ใช่เลขในไฟล์ Excel
คำว่า "Excel" ทำให้ admin เข้าใจมาตลอดว่ามันเท่ากับยอดในไฟล์ — ทั้งที่ในรอบจริง
**56–74% ของ SKU มีสองค่านี้ไม่เท่ากัน** · ชื่อใหม่คือ **"ยอดตั้งต้นตอนนี้"**
(แถวสรุปในแผงเขียนเต็มว่า `ยอดตั้งต้นตอนนี้ (ไฟล์+ปรับ)`) · มีเทสห้ามคำเก่ากลับมาทั้งไฟล์

| รอบ | SKU | ที่ `book_qty` ≠ `effective_book_qty` |
|---|---|---|
| คลังอะไหล่ 2026-06 | 168 | 125 (74%) |
| ตึกกันตนา+หน้าไลฟ์ 2026-05 | 1,552 | 988 (64%) |
| ตึกกันตนา 2026-08 | 1,114 | 618 (56%) |

### ⛔ หมายเหตุของ acceptance ต้องมาจากยอดที่บันทึกจริง ไม่ใช่ช่องกรอก

`buildMarkMatchNote(effective, counted, target)` — ของเดิมจด `ยืนยันถูกต้อง @ ${target}`
โดย `target` คือเลขในช่องซึ่ง **ตั้งค่าเริ่มต้นเป็นผลนับ** ⇒ จดเลขที่ถูก *ปฏิเสธ* ไว้แทน
**ข้อมูลเสียจริง 23 จาก 76 แถว** (เช่น `SP069` จด `@30` ทั้งที่ระบบบันทึก 68)
ตอนนี้จด `ใช้ยอดตั้งต้น N เป็นยอดจริง (ผลนับ M)` และถ้าเลขในช่องต่างจาก N จะต่อท้าย
`· เลขในช่องตอนกด X ไม่ถูกใช้` เก็บเป็นหลักฐาน · **ข้อมูลเก่ายังไม่ได้แก้**

### ⚠️ `Number('') === 0` — ใช้ `parseQtyOrNull()` เสมอ

ช่องกรอกที่ว่างจะถูกอ่านเป็น "ยอด 0" ถ้าเช็คแค่ `Number.isFinite(Number(v))`
พลาดมาแล้ว 2 ครั้งในวันเดียว (ตัวช่วยเลือกปุ่ม + หมายเหตุ) จึงรวมเป็น helper ตัวเดียว
