...
| **ใช้ยอดที่กรอกเป็นยอดจริง (Apply ทันที)** — ปุ่มกด 2 ครั้ง (armed 8 วิ) · ปรับ Excel ใช้เทียบเป็นยอดที่กรอก · **ถ้ายอดที่กรอกต่างจากผลนับ จะเขียน `reconciliation_match_acceptances` กำกับอัตโนมัติ** (ไม่งั้นสถานะค้างขาด/เกินทั้งที่คนตัดสินแล้ว — แก้ 2026-08-13 พร้อมเปลี่ยนชื่อจาก "ยอมรับผลนับ") | `acceptCountedForLine` |
| **ยืนยันว่ายอด Excel ถูกต้อง (ไม่ปรับยอด)** → `reconciliation_match_acceptances` (ชื่อเดิม "ยืนยันเป็นถูกต้อง" — เปลี่ยน 2026-08-13 เพราะ admin เข้าใจกลับด้านว่าปุ่มนี้เชื่อผลนับ) | `markLineAsMatchAccepted` |
